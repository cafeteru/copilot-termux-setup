...
# Verify
COPILOT_BIN="$(npm root -g)/@github/copilot/npm-loader.js"
if [ -f "$COPILOT_BIN" ]; then
    echo "==> Copilot CLI installed successfully"
    # Try to get version
    node "$COPILOT_BIN" --version 2>/dev/null || true
else
...
PROOTSCRIPT
    ...
    if proot-distro login "$PROOT_DISTRO" -- bash "$PROOT_SETUP_SCRIPT"; then
...
exec proot-distro login $PROOT_DISTRO -- bash -c 'cd "\$(pwd 2>/dev/null || echo \$HOME)" && copilot "\$@"' -- "\$@"
EOF
